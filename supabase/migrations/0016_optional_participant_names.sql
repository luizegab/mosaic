-- Name and email are now ordinary form questions the organizer may remove,
-- so a participant can legitimately arrive with no name. Drop the
-- name-required check and null-proof the inserts (first_name/last_name are
-- NOT NULL columns; missing JSON fields must become empty strings).
-- Identity extraction/validation lives in /api/register, the only caller.
