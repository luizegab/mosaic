 ...
 * Falls back: requested locale → fallback (default) locale → ''.
 *
 * It intentionally does NOT fall back to "first available value": doing so
 * would show a random other language (e.g. Cyrillic text on a French page)
 * whenever a field is missing the requested locale — the "mixed alphabets"
 * bug. A missing translation shows the event's default language instead.
 */
export function lt(value, locale, fallbackLocale = DEFAULT_LOCALE) {
  if (value == null) return ''
  if (typeof value === 'string') return value
  const requested = value[locale]
  if (requested != null && String(requested).trim() !== '') return requested
...
/**
 * Short label for a language code — "EN", "UK", "PT".
 *
 * Used on the pages attendees see, where the switcher sits in a hero beside a
 * logo and a full name like "Українська" costs more room than it earns. The
 * console keeps full names: an organizer is choosing which language to author
 * in and needs to be sure, whereas an attendee is finding their own language in
 * a list they already recognize.
 *
 * The code itself, uppercased — the convention every multilingual site uses —
 * rather than an abbreviation of the translated name, which would differ per
 * script and stop being recognizable.
 */
export function localeAcronym(code) {
